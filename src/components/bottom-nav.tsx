...
export function BottomNav() {
  ...
  return (
    <nav className="pointer-events-none fixed inset-x-0 bottom-0 z-40 flex justify-center pb-[max(env(safe-area-inset-bottom),0.75rem)]">
      <div className="pointer-events-auto mx-4 flex w-full max-w-md items-center justify-between rounded-full border border-border/60 bg-surface/95 p-1.5 shadow-[0_16px_40px_-16px_rgba(76,47,115,0.35)] backdrop-blur">
          ...
          return (
            <Link
              ...
              className={`flex flex-1 items-center justify-center gap-2 rounded-full px-3 py-2.5 text-[12px] font-bold transition-all ${
                active
                  ? "bg-indigo-deep text-white shadow-[0_10px_20px_-10px_rgba(76,47,115,0.6)]"
                  : "text-ink-soft"
              }`}
            >
